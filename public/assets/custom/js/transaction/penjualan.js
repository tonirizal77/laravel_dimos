...
$(function () {
    ...
    form_Jual.enterAsTab({ allowSubmit: false });
                    ...
                    for (let x = 0; x < data.length; x++) {
                        ...
                        let rowdata =
                            `<tr tabindex="` +nobrs +`" data-sesi="` +data[x].kode_sesi +`" class="text-center">
                                ...
                                <td>` +data[x].kasir+`</td>
                                <td>` +int2string_number.format(data[x].no_trx) +`</td>
                            </tr>`;
    ...
    tabel_barang
        ...
        .on("change", "td select.select-in", function () {
            ...
            if (getNilai >= 1) {
                ...
                harga.text(int2string_number.format(hrgj));
            }
        });
    ...
    kode_Brg
        ...
        .on("focus", function () {
            ...
            $("#harga_x").text("");
            $("#jml_x").text(int2string_number.format(grand_total));
        });
    ...
    function loadDataBarang() {
        $.ajax({
            url: "/admin/master-data/products/load-data",
            type: "GET",
            dataType: "json",
            beforeSend: function() {
                // setting a timeout
                    ...
                    for (let i = 0; i < data.length; i++) {
                        ...
                        let rowdata =
                            `<tr tabindex="` +data[i].id +`" data-id="` +data[i].id +`">`+
                                ...
                                `<td>` +data[i].name +`</td>`+
                                `<td> `+ satuan + `</td>`+
                                `<td class="harga text-right">` +int2string_number.format(data[i].harga_jual) +`</td>`+
                            `</tr>`;
     ...
     */
     function buatSatuan(p1,p2,p3,p4,p5,p6) {
        const h_dasar = p1;
        const s_dasar = p2;
        const kvsi = p3;
        const nl_kvsi = p4;
        ...
        if (kvsi != null && nl_kvsi != null) {
            ...
            if (satkonv.length > 1) {
                ...
                c_select.append(`
                    <option data-dasar="`+h_dasar+
                        `" data-opr="*" data-nilai="1" data-kode="`+s_dasar+
    ...
    tabel_rinci
        ...
        .on("change", "td select.select-in2", function () {
            ...
            let qty_item = row.find("input.qty_jual");
            ...
            // if (getNilai >= 1) {
                // console.log("Lakukan Proses Konversi Harga Jual");
                // update harga dan jumlah di tabel_rinci
                harga.val(int2string_number.format(hrg_dasar));
                jumlah.val(int2string_number.format(hrg_dasar*qty_item.val()))
                totalForm();
            // }

            //update value form jika opt_rinci = "UPDATE"
            if (opt_rinci == "UPDATE" && row.hasClass("selected-row")) {
                hrg_jual.val(int2string_number.format(hrg_dasar))
                satuan.val($(this).val())
                satuan.trigger("change")// toni
                $("span#label_satuan").text($(this).val());
                tot_harga.val(int2string_number.format(hrg_dasar*qty_item.val()))
            }
    ...
    // Update Item
    btn_ActionForm.on("click", "button#btn-update", function () {
        opt_rinci = "UPDATE";
    ...
    // Batal Update Item
    btn_ActionForm.on("click", "button#btn-cancel", function () {
        //reset
        opt_rinci = "ADD";
    ...
    satuan
        .on("change", function () {
            ...
            if (getNilai >= 1) {
                ...
                hrg_jual.val(hrgj);
                tot_harga.val(qty_item.val() * hrgj);
            }
            // toni
            $("span#label_satuan").text($(this).val());
        })
    ...
    function daftar_nota(param1, param2) {
        $.ajax({
            ...
            success: function (resp) {
                ...
                $("#total_sesi_info").text(int2string_number.format(total_all))
                    ...
                    for (let i = 0; i < rinci.length; i++) {
                        ...
                        rincian_Data.append(
                            `<tr tabindex="` +no_item +`" data-id="` +no_item +`">
                                ...
                                <td>
                                    <input type="text" name="harga_brg[]" value="` +int2string_number.format(rinci[i].harga_jual) +`" class="harga_brg form-control text-right editable rupiah" readonly>
                                </td>
                                <td>
                                ...
                                <td>
                                    <input type="text" name="jml_harga[]" value="` +int2string_number.format(rinci[i].harga_jual * rinci[i].qty) +`" class="jml_harga form-control text-right rupiah" disabled>
                                </td>
                            </tr>
                            `
                        );
    ...
    function tambah_Item() {
        ...
        let harga = toDouble(hrg_jual.val());
        ...
        if (jumlah > 0) {
            ...
            //cari item yg sama kode
            let elm = rincian_Data.find('tr');
            if (elm.length > 0) {
                for (let x = 0; x < elm.length; x++) {
                    const e = $(elm[x]).find('td');
                    let e_kode = $(elm[x]).find('td input.kode_brg').val();
                    let e_sat = $(elm[x]).find('td select.satuan').val();

                    let harga = $(elm[x]).find('td input.harga_brg').val();
                    let qty = $(elm[x]).find('td input.qty_jual').val();
                    let jmlhrg = $(elm[x]).find('td input.jml_harga').val();
                    let sat = $(elm[x]).find('td select.satuan').val();

                    if ( $.trim(e_kode) == $.trim(kode_Brg.val()) &&
                        $.trim(e_sat) == $.trim(satuan.val()) ) {
                        console.log(e);
                    }
                }
            }
            ...
            rincian_Data.append(
                `<tr tabindex="` +no_item +`" data-id="` +no_item +`">
                    <td class="text-center no_urut">` +tot_item +`</td>
                    <td>
                        <input type="text" name="kode_brg[]" value="` +$.trim(kode_Brg.val()) +`" class="kode_brg form-control text-center " readonly>
                    </td>
                    <td>
                        <input type="text" name="nama_brg[]" value="` +nama_Brg.val() +`" class="nama_brg form-control" disabled>
                    </td>
                    <td>
                        <input type="text" name="harga_brg[]" value="` +int2string_number.format(harga) +`" class="harga_brg form-control text-right editable rupiah" readonly>
                    </td>
                    <td>
                        <div class="form-group row m-0 justify-content-between">
                            <div class="col-sm-5 p-0 qty">
                                <input type="number" name="qty_jual[]" value="` +toNumber(qty_item.val()) +`" class="qty_jual form-control text-right editable" readonly>
                            </div>
                            <div class="col-sm-6 p-0 satuan">
                                <select name="satuan[]" class="satuan select-in2 form-control text-sm" readonly>
                                    `+satuan.html()+`
                    ...
                    <td>
                        <input type="text" name="jml_harga[]" value="` +int2string_number.format(jumlah) +`" class="jml_harga form-control text-right rupiah" disabled>
                    </td>
                </tr>`
            );
            ...
            mynotife(
                "success",
                "Sukses Tambah Item",
                nama_Brg.val() + "<br>Sukses di-tambahkan"
            );
    ...
    function hapus_Item(target) {
        ...
        Swal.fire({
            ...
            html:
                ...
                nama + "</b><br>Qty : <b>" +
                qty + " " +
                sat + "  x (Rp. " + int2string_number.format(toNumber(hrg)) + ")",
    ...
    //Input Pembayaran
    $("div.inputBayar").on("input", function () {
    ...
    function totalForm() {
        ...
        let qty = qty_item.val();
        let jml = toDouble(hrg_jual.val());
        tot_harga.val(qty * jml); // update nilai total harga per Item
    ...
    form_Jual.validate({
        ignore: ".ignore",
        rules: {
            code_item: { required: true, minlength: 6, maxlength: 13 },
            nama_item: { required: true },
            hrg_jual: { required: true, lebihBesar: "1000", number: true },
        ...
        submitHandler: function (form) {
            ...
            $.ajax({
                ...
                success: function (resp) {
                    ...
                    // reset
                    formChange();
                    mynotife(pesan, "Simpan Transaksi", isi);
                },
            });
        },
    });
